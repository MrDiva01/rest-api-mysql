...
userRouter.get("/users/search", async (req: Request, res: Response) => {
    try {
        ...
        const query = { name: name as string, email: email as string };

        const matchedUsers = await database.searchUsers(query);

        return res.status(StatusCodes.OK).json({ total_users: matchedUsers.length, matchedUsers });
    } catch (error) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error });
    }
});
